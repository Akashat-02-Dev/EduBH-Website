...
export const Header = ({ setUiHover }: { setUiHover: (v: boolean) => void }) => {
  ...
  return (
    <Navbar className="fixed top-0 z-40" onMouseEnter={() => setUiHover(true)} onMouseLeave={() => setUiHover(false)}>
      {/* Desktop Navigation */}
      <NavBody>
        <NavbarLogo />
        <NavItems
      ...
      <MobileNav>
        ...
        <MobileNavMenu isOpen={isOpen} onClose={() => setIsOpen(false)}>
          <div className="flex flex-col gap-2 p-4 bg-white dark:bg-neutral-900 rounded-2xl shadow-lg w-full">
            ...
            <NavbarButton variant="dark" className="w-full mt-2 bg-[#1A3EC3] hover:bg-[#2546a8]" href="/apply">
              Enquire Now
            </NavbarButton>
          </div>
        </MobileNavMenu>
