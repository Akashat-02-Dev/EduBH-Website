...
export const Carousel = ({ items, initialScroll = 0 }: CarouselProps) => {
  ...
  useEffect(() => {
    if (isPaused) return;
    if (typeof window !== 'undefined' && window.innerWidth < 640) return; // Disable auto-scroll on mobile
    let frameId: number;
    const getScrollStep = () => {
    ...
    const step = () => {
      if (carouselRef.current) {
        ...
        // If at the end, reset to start for seamless loop
  ...
  // Use only the original items (no duplication)
  return (
